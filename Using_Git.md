...
Git, TortoiseGit and EGit provide some tools to assist with merging.  (The TortoiseGit "Resolve" command is **not** one of those tools; see below.)  However, in some cases, it may be simple to manually merge the changes.  To manually merge, you'll need to edit any conflicting files, look for conflict markers (<<<<<< and >>>>>>), and correct the file.
...
Finally, when you're ready to commit your merged changes, you will first need to mark the conflict as being resolved.  You can do this by using **TortoiseGit > Resolve** or EGit **Team > Add**.  Note that these commands **do not** resolve your conflict for you; they simply inform git that the conflict has been resolved.
